...
class TianyanSpider():
    ...
    def search(self,obj,names,num,l,status):
        i = 0
        index = 1
        # 按公司名进行搜索
        for name in names:
            ...
            if i >= 50:
                #  退出登录
                info = obj.driver.find_element_by_xpath('//a[@class="title link-white"]')
                ActionChains(obj.driver).move_to_element(info).perform()
                time.sleep(1)
                out = obj.driver.find_element_by_xpath('//div[@class="block-content"]/a[@class="item"][7]')
                obj.driver.execute_script('arguments[0].click();',out)
                if index >= l:
                    index = 0
                time.sleep(2)
                obj = obj.start(index)
                index += 1
                i = 0
            obj.driver.find_elements_by_xpath('//input[@type="search"]')[0].send_keys(name.strip())
            time.sleep(random.randint(1,3))
            try:
                obj.driver.find_elements_by_xpath('//div[@class="name js-text"]')[0].click()
            except (NoSuchElementException,IndexError):
                obj.driver.find_elements_by_xpath('//input[@type="search"]')[0].clear()
                num += 1
                continue
            except StaleElementReferenceException:
                obj.driver.quit()
                sys.exit('请重新运行代码')
            self.fanhui(obj.driver,num,name,status)
            num += 1

    def fanhui(self,driver,num,name,status):
        # 切换到新的页面进行数据的提取
        window = driver.window_handles
        driver.switch_to.window(window[1])
        self.extract(driver,num,name,status)
        # 关闭新页面，切换到原页面输入下一个公司的名称进行查询
        driver.close()
        driver.switch_to.window(window[0])
        driver.find_elements_by_xpath('//input[@type="search"]')[0].clear()
        time.sleep(random.randint(1,2))

    def extract(self,driver,num,name,status):
        # 进行数据的提取
        time.sleep(1)
        textlist = [str(num)]
        try:
            ...
            while True:
                try:
                    driver.find_element_by_xpath('//div[@class="new-box94"]')
                except NoSuchElementException:
                    return
                time.sleep(3)
                continue
        name = driver.find_element_by_xpath('//div[@class="header"]/h1').text
        ...
        # 获取运营状态
        try:
            zhuangtai = driver.find_element_by_xpath('//div[@class="tag-list"]/div[1]').text
        except NoSuchElementException:
            return
        ...
        try:
            info = driver.find_element_by_xpath('//div[@class="data-content"]/table[2]')
        ...
        # 法人
        try:
            Representative = driver.find_element_by_xpath('//div[@class="name"]/a').text
            textlist.append(Representative)
        except NoSuchElementException:
            return

        # 电话
        photo = driver.find_element_by_xpath('//div[contains(@class,"sup-ie-company-header-child-1")]/span[2]').text
        if len(photo) == 11:
            textlist.append('/')
        ...
        # 知识产权，资质证书
        certificate = driver.find_elements_by_xpath('//div[@id="nav-main-knowledgeProperty"]/div[@class="block-data"]/div/span')
        if certificate == []:
        ...
        property = driver.find_elements_by_xpath('//div[@id="nav-main-manageStatus"]/div[@class="block-data"]/div/span')
        if property == []:
        ...
        # 邮箱
        email = driver.find_element_by_xpath('//div[contains(@class,"sup-ie-company-header-child-2")]/span[2]').text
        textlist.append(email)

        # 网址
        try:
            url = driver.find_element_by_xpath('//div[contains(@class,"sup-ie-company-header-child-1")]//a').get_attribute('href')
        except NoSuchElementException:
        ...
        # 地址
        try:
            address = driver.find_element_by_xpath('//div[contains(@class,"sup-ie-company-header-child-2")]/div/div').text
        except NoSuchElementException:
            textlist.append('暂无信息')
        else:
    ...
    def main(self):
        ...
        names = self.read(number,name)
        l = self.read_txt()
        obj = self.login()
        self.search(obj,names,number,l,status)
        self.write(name)
